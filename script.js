const urlBase = 'https://jsonplaceholder.typicode.com/posts/'; // url para consumir la api
let posts = [] // iniciamos los posteos como array vacio
...
function postData() {
    ...
    if (postTitle.trim() == '' || postBody.trim() == '') {
        alert('Los campos con obligatorios')
        return
    }
...
function updatePost(id) {
    ...
    fetch(`${urlBase}/${id}`, {
        ...
        .catch(error => {
            console.error('Error al quere actualizar el posteo', error);
        })
}
...
function deletePost(id) {
    ...
    .then(res => {
        if(res.ok){
            ...
            alert('Hubo un error y no se pude eliminar el post')
        }
    })
    .catch(error => console.error('Hubo un error eliminando'));
}